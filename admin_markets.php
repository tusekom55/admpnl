...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action'])) {
        try {
            switch($_POST['action']) {
                case 'add_symbol':
                    $query = "INSERT INTO markets (symbol, name, current_price, change_percent, is_active) VALUES (:symbol, :name, :current_price, :change_percent, 1)";
                    ...
                    $stmt->execute();
                    $success_message = "Sembol başarıyla eklendi!";
                    break;
                    
                case 'update_price':
                    $query = "UPDATE markets SET current_price = :current_price, change_percent = :change_percent, last_updated = NOW() WHERE id = :id";
                    $stmt = $db->prepare($query);
                    $stmt->bindParam(':current_price', $_POST['current_price']);
                    $stmt->bindParam(':change_percent', $_POST['change_percent']);
...
// Sembol listesi
$query = "SELECT * FROM markets ORDER BY symbol ASC LIMIT :limit OFFSET :offset";
$stmt = $db->prepare($query);
...
<body>
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    $<?php echo number_format($market['current_price'], 4); ?>
                                </td>
                                <td>
                                    <span class="badge <?php echo $market['change_percent'] >= 0 ? 'bg-success' : 'bg-danger'; ?>">
                                        <?php echo $market['change_percent'] >= 0 ? '+' : ''; ?><?php echo number_format($market['change_percent'], 2); ?>%
                                    </span>
                                </td>
                                <td>
                                ...
                                <td>
                                    <button class="btn btn-sm btn-primary" onclick="editPrice(<?php echo $market['id']; ?>, '<?php echo $market['symbol']; ?>', <?php echo $market['current_price']; ?>, <?php echo $market['change_percent']; ?>)">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" step="0.0001" class="form-control" name="current_price" id="edit_current_price" required>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label">Değişim %</label>
                                    <input type="number" step="0.01" class="form-control" name="change_percent" id="edit_change_percent" required>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    function editPrice(id, symbol, currentPrice, changePercent) {
        ...
        document.getElementById('edit_change_percent').value = changePercent;
